...
function ExperienceCard({ exp }) {
  ...
  return (
    <div className="scroll-hidden relative group">
      ...
      <div className="rounded-[28px] border transition-all duration-400 overflow-hidden relative"
        style={{
          background: 'var(--card-bg)',
          borderColor: exp.current ? 'rgba(79,70,229,0.2)' : 'var(--border-solid)',
          boxShadow: 'var(--shadow-md)',
        }}>
        {/* Top progress line — visible by default when current */}
        {exp.current && (
        ...
        <div className="p-7">
          ...
          <div className="space-y-3 mb-6">
            {visiblePoints.map((point, i) => (
              <div key={i} className="flex items-start gap-3 text-sm leading-relaxed font-semibold" style={{ color: 'var(--text-sub)' }}>
                <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 text-white text-[9px] font-bold"
          ...
          {exp.points.length > 2 && (
            <button
              ...
              className="text-xs font-bold mb-5 flex items-center gap-1 transition-colors hover:opacity-80"
          ...
          {/* Tech tags */}
          <div className="pt-5 flex flex-wrap gap-2" style={{ borderTop: '1px solid var(--border)' }}>
            {exp.technologies.map(tech => (
              <span key={tech}
                className="px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-tight border transition-all duration-200 hover:-translate-y-0.5"
...
export default function Experience() {
  return (
    <section id="experience" className="py-16 px-6 relative" style={{ background: 'var(--section-alt)' }}>
